...
(function(root, factory) {
  ...
  return function(opts) {
    ...
    if (this.options.username == "" && this.options.tag == "") {
      ...
      console.error(
        "InstagramFeed: Error, invalid or undefined callback for get_data"
      );
      this.valid = false;
    }
      ...
      xhr.onload = function(e) {
        if (xhr.readyState === 4) {
          if (xhr.status === 200) {
            ...
            data =
              data.entry_data.ProfilePage || data.entry_data.TagPage || null;
            if (data === null) {
              console.log(url);
              console.error(
                "InstagramFeed: Request error. No data retrieved: " +
                  xhr.statusText
              );
            } else {
            ...
            console.error(
              "InstagramFeed: Request error. Response: " + xhr.statusText
            );
          }
        }
      };
    ...
    this.parse_caption = function(igobj, data) {
      if (
        ...
        igobj.node.edge_media_to_caption.edges[0].node.text.length != 0
      ) {
    ...
    this.display = function(data) {
      ...
      if (this.options.styling) {
        var width =
          (100 - this.options.margin * 2 * this.options.items_per_row) /
      ...
      if (this.options.display_profile) {
        html +=
          "<div class='instagram_profile'" + styles.profile_container + ">";
        html +=
      ...
      if (this.options.display_gallery) {
        ...
        if (
          typeof data.is_private !== "undefined" &&
          ...
          var imgs = (
            data.edge_owner_to_timeline_media || data.edge_hashtag_to_media
          ).edges;
          max =
            imgs.length > this.options.items ? this.options.items : imgs.length;
      ...
      if (
        ...
        var igtv = data.edge_felix_video_timeline.edges,
          max =
            igtv.length > this.options.items ? this.options.items : igtv.length;
        if (igtv.length > 0) {
